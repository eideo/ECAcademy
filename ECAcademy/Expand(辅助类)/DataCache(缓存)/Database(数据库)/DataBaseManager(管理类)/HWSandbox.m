...
//  Sqlite_teset_01
//
//  Created by yellowei on 15/12/1.
//  Copyright (c) 2015年 yellowei. All rights reserved.
//

#import "HWSandbox.h"

@implementation HWSandbox
...
//    NSString* myCache = [[paths objectAtIndex:0] stringByAppendingFormat:@"/Caches"];
    NSString* myCache = [paths objectAtIndex:0];
    
    NSFileManager * mg = [[NSFileManager alloc] init];
    if (![mg fileExistsAtPath:myCache])
    {
        [mg createDirectoryAtPath:myCache withIntermediateDirectories:NO attributes:nil error:nil];
    }
...
#ifdef Cache_db
    NSString* documentsPath = [documentsDirectory stringByAppendingPathComponent:@"/Cache_db"];
    
    
    
    NSFileManager * mg = [[NSFileManager alloc] init];
    if (![mg fileExistsAtPath:documentsPath])
    {
        [mg createDirectoryAtPath:documentsPath withIntermediateDirectories:NO attributes:nil error:nil];
    }
